...
static void dummy(const std::shared_ptr<Channel>& chn)
{
...
size_t Request::headerData(char *buffer, size_t size, size_t nmemb, void *userp)
{
    assert(size == 1);
    Request* req = static_cast<Request*>(userp);
    req->headerCallback(buffer, nmemb);
    return nmemb;
}
//----------Curl class

void Curl::initialize(Curl::Option opt)
{
    curl_global_init(opt == Curl::Option::kCURLnossl ? CURL_GLOBAL_NOTHING: CURL_GLOBAL_SSL);
}
...
)
{
    ...
    LOG_DEBUG << "Curl::socketCallback [" << curl << "] - fd = " << fd << " what = " << whatstr[what];
    Request *req = NULL;
